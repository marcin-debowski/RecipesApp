...
export default function Something() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Zapisz token, jeśli jest zwracany
      localStorage.setItem("token", data.data.token);
      // Przekieruj użytkownika
      navigate("/page1");
    } catch (err) {
      setLoginError("Server error" + err);
    }
  };

  // Dodaj stan do rejestracji
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Opcjonalnie automatycznie zamknij popup po kilku sekundach
      setTimeout(() => {
      ...
      setRegisterError("Server error: " + err);
    }
  };
      ...
      >
        ...
        {showRegister && (
            <div
                ...
                onClick={() => setShowRegister(false)} // zamyka popup po kliknięciu w tło
            >
              <div
                  className="bg-white rounded-3xl shadow-2xl p-6 sm:p-10 w-full max-w-md flex flex-col items-center relative"
                  onClick={e => e.stopPropagation()} // zatrzymuje propagację kliknięcia, więc kliknięcie w okno nie zamyka popupu
              >
                <button
